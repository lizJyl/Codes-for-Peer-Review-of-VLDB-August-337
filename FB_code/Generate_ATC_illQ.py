...
def load_data(path="../facebook/"):
	...
	f1 = open("{}/ATC_illQ/{}.query.node".format(path, ego), 'w')
	...
	np.savetxt("{}/ATC_illQ/{}.sample".format(path,ego), inputs, fmt='%d')
	f1.close()

	allattr = np.concatenate(allattr, axis=0)
	print("allattr size", allattr.shape, allattr.dtype)
	np.savetxt("{}/ATC_illQ/{}.sample.attri".format(path, ego), allattr, fmt='%d')


	labels=[]
	querynum=0
	count=0
	f = open("{}/ATC_illQ/{}.query.com".format(path, ego), 'w')
	for i in range(circles.shape[0]):
		if len(circles[i])<5:
			continue
		for k in range(len(circles[i])//2-1):
	...
	f.close()
	np.savetxt("{}/ATC_illQ/{}.label".format(path,ego), labels, fmt='%d')
	...
	f = open("{}/ATC_illQ/{}.graph".format(path, ego), 'w')
	f2 = open("{}ATC_illQ/{}.keywords".format(path, ego), 'w')
	edge = []
