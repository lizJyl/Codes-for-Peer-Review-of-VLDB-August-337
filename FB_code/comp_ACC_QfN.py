...
def load_data(path="../facebook/", ego=107):
    ego = 414
    ego = 686
    ego = 348
    ego = 0
    ...
    file = open("{}ACC_QfN/result/ACC_QfN_{}.result".format(path, ego), 'r')
    ...
    file = open("{}ACC_QfN/{}.query.com".format(path, ego), 'r')
    val_list = file.readlines()
    # val_list=val_list[:,0:-1]
    lists = []
    for string in val_list:
    ...
    for i in range(ground_truth.shape[0]):
        ...
        for k in range(ACC_result[j].shape[0]):
            result.add(int(ACC_result[j][k])-1)
        j=j+1
