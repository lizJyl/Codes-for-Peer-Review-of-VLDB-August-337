...
def load_data(path="../facebook/"):
	...
	f1 = open("{}/ATC/{}.query.node".format(path, ego), 'w')
	...
	np.savetxt("{}/ATC/{}.sample".format(path,ego), inputs, fmt='%d')
	f1.close()
	# np.savetxt("{}/ATC/{}.query.node".format(path, ego), qeury_node, fmt='%d')


	labels=[]
	querynum=0
	count=0
	f = open("{}/ATC/{}.query.com".format(path, ego), 'w')
	for i in range(circles.shape[0]):
		if len(circles[i])<5:
			continue
		for k in range(len(circles[i])//2-1):
	...
	f.close()
	np.savetxt("{}/ATC/{}.label".format(path,ego), labels, fmt='%d')
	...
	f = open("{}/ATC/{}.graph".format(path, ego), 'w')
	f2 = open("{}ATC/{}.keywords".format(path, ego), 'w')
	edge = []
