...
def load_data(path="../facebook/"):
	...
	f1 = open("{}ATC_QfN/{}.query.node".format(path, ego), 'w')
	...
	np.savetxt("{}ATC_QfN/{}.sample".format(path,ego), inputs, fmt='%d')
	f1.close()

	allattr = np.concatenate(allattr, axis=0)
	print("allattr size", allattr.shape, allattr.dtype)
	np.savetxt("{}ATC_QfN/{}.sample.attri".format(path, ego), allattr, fmt='%d')
	...
	querynum=0
	allcount=0
	f = open("{}ATC_QfN/{}.query.com".format(path, ego), 'w')
	for i in range(circles.shape[0]):
		if allcount > 349:
			break
		count = 0
	...
	f.close()
	np.savetxt("{}ATC_QfN/{}.label".format(path,ego), labels, fmt='%d')
	...
	f = open("{}ATC_QfN/{}.graph".format(path, ego), 'w')
	f2 = open("{}ATC_QfN/{}.keywords".format(path, ego), 'w')
	edge = []
